...
def loginUser(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            login(request, user)
            # redirects the user to the project that they wanted to review if they weren't logged in
...
def inbox(request):
    profile = request.user.profile
    # remember the variable can't be messages because we import both Message and messages already
    # profile.messages(recipient) in models
